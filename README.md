...
실행기는 필요할 때 모델과 추론 수준을 지정해 격리된 Docker 환경에서 Skill을 호출하고, 모델 실행이 끝난 뒤 검증기를 강제하는 역할을 담당합니다. 이를 통해 모델의 지시 준수, 도구 활용, 오류 복구 및 결과 일관성을 실제 주간 리포트 생성 과정에서 관찰할 수 있습니다.
